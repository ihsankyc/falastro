...
function stickyHeader() {
  if ($(".stricky").length) {
    ...
    if ($(window).scrollTop() > strickyScrollPos) {
      stricky
        .removeClass("slideIn animated")
        .addClass("stricky-fixed slideInDown animated");
      $(".scroll-to-top").fadeIn(500);
    } else if ($(this).scrollTop() <= strickyScrollPos) {
      stricky
        .removeClass("stricky-fixed slideInDown animated")
...
function mobileNavToggle() {
  if ($(".navbar-collapse li.dropdown ul").length) {
    $(".navbar-collapse li.dropdown").append(
...
function updateLink() {
  var os = detectOS();
  var link = document.getElementById("downloadApp");
  if (os === "iOS") {
    // iOS App Store URL
